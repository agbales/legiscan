...
export const fetchBill = async (billId, apiKey) => {
    ...
    try {
        ...
        console.log('Fetched bill:', billId);
        return res.bill;
    }
    catch (error) {
...
const fetchSearch = async (query, apiKey, page, state) => {
    const op = 'getSearch';
    try {
...
async function getPaginatedSearchResults(query, apiKey, state, page = 1, accumulatedResults = []) {
    ...
    console.log('Added ', res.results.length, ' results to response');
    return getPaginatedSearchResults(query, apiKey, state, page + 1, updatedResults);
}
export const searchWithPagination = async (query, apiKey, state) => {
    const searchState = state ? state : 'ALL';
    const response = await getPaginatedSearchResults(query, apiKey, searchState);
    return response;
};
